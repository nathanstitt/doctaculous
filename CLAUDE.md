...
- **HTML rendering — end-to-end "specimen" showcase** (`testdata/htmldoc/` fixture tree + `pkg/doctaculous`
  `htmldoc-p{0..7}` goldens via `TestHTMLDocShowcase`): a single multi-file document that exercises **every**
  ...
  positioning (relative/absolute) + overflow clip + z-index stacking, and PNG/JPEG/GIF images with
  `object-fit`/`object-position`. It is served over **loopback HTTP** (`net/http/httptest` + `http.FileServer`)
