...
func TestDirLoaderRefusesTraversal(t *testing.T) {
	dir := t.TempDir()
	// A file OUTSIDE the base dir. "../secret.txt" has its single leading "../"
	// trimmed by the guard and re-rooted to <base>/secret.txt (inside, nonexistent),
	// so this refuses because the in-base retry target is absent. TestDirLoaderRefusesGenuineEscape
	// exercises the guard's true refusal branch (a cleaned target that still escapes base).
...
// TestDirLoaderRefusesGenuineEscape proves the guard's refusal branch by pointing a
// ref at a file that DOES exist OUTSIDE Base, via a path whose cleaned target escapes
// Base even after the guard trims leading "../". The guard trims only leading "../"
// segments, so "../x/../../secret.txt" becomes "x/../../secret.txt", which joined
// against base cleans UP two levels to <parent>/secret.txt — outside base — so it is
// refused. If the guard were broken to serve genuinely-escaping paths, this test would
// FAIL (it would read the real outside file), unlike the "../../etc/passwd" assertion
// which passes merely because that path is absent.
func TestDirLoaderRefusesGenuineEscape(t *testing.T) {
	dir := t.TempDir()
	// A real, existing file in Base's PARENT directory.
	outside := filepath.Join(filepath.Dir(dir), "secret.txt")
	...
	d := DirLoader{Base: dir}

	// Cleans (after the guard trims the leading "../") to <parent>/secret.txt — outside
	// base — so it must be refused even though the target file exists.
	ref := "../x/../../secret.txt"
	got, _, err := d.Load(context.Background(), ref)
	if !errors.Is(err, ErrNotFound) {
		t.Fatalf("%s: err = %v, want ErrNotFound (genuinely-escaping ref must be refused)", ref, err)
	}
	if got != nil {
		t.Fatalf("%s: served %q bytes, want none (must not read the outside file)", ref, got)
	}
}
