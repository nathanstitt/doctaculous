...
func EncodeImageRect(out io.Writer, img image.Image, opts ImageOptions) (image.Rectangle, error) {
	...
	if opts.Crop != nil {
		...
		if err != nil {
			return image.Rectangle{}, fmt.Errorf("omnidoc: crop image: %w", err)
		}
		img, used = cropped, r
	}
...
func encodeImageTo(out io.Writer, img image.Image, opts ImageOptions) error {
	...
	case FormatPNG:
		if err := png.Encode(out, img); err != nil {
			return fmt.Errorf("omnidoc: encode png: %w", err)
		}
	case FormatJPEG:
		if err := jpeg.Encode(out, img, &jpeg.Options{Quality: opts.quality()}); err != nil {
			return fmt.Errorf("omnidoc: encode jpeg: %w", err)
		}
	case FormatWebP:
		// Lossless VP8L; opts.Quality does not apply. See webp.Encode.
		if err := webp.Encode(out, img); err != nil {
			return fmt.Errorf("omnidoc: encode webp: %w", err)
		}
	default:
		return fmt.Errorf("omnidoc: encode image: %s is not an image format: %w", f, ErrUnsupportedFormat)
	}
	return nil
}
