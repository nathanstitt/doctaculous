...
func Open(path string, opts ...OpenOption) (*Document, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("omnidoc: open %q: %w", path, err)
	}
...
func OpenReader(ctx context.Context, r io.Reader, opts ...OpenOption) (*Document, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("omnidoc: read input: %w", err)
	}
...
func OpenAs(f Format, path string, opts ...OpenOption) (*Document, error) {
	if isHTTPURL(path) {
		...
		return nil, fmt.Errorf("omnidoc: open %q: an http(s) URL is fetched as a web page (HTML), not %s", path, f)
	}
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, fmt.Errorf("omnidoc: open %q: %w", path, err)
...
func openDetected(ctx context.Context, f Format, data []byte, dir string, opts []OpenOption) (*Document, error) {
	...
	default:
		return nil, fmt.Errorf("omnidoc: cannot detect the document format (open with an explicit format via OpenAs or OpenReaderAs, or use a recognizable file extension): %w", ErrUnknownFormat)
	}
}
