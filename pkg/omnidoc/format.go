...
func CanConvert(from, to Format) error {
	if _, ok := formatCaps[from]; !ok {
		return fmt.Errorf("omnidoc: input format %q: %w", string(from), ErrUnknownFormat)
	}
	if _, ok := formatCaps[to]; !ok {
		return fmt.Errorf("omnidoc: output format %q: %w", string(to), ErrUnknownFormat)
	}
	if !from.ValidInput() {
		return fmt.Errorf("omnidoc: %s is not a supported input format: %w", from, ErrUnsupportedFormat)
	}
	if !to.ValidOutput() {
		return fmt.Errorf("omnidoc: %s is not a supported output format: %w", to, ErrUnsupportedFormat)
	}
	if from == to {
		return fmt.Errorf("omnidoc: cannot convert %s to %s: %w", from, to, ErrSameFormat)
	}
	return nil
}
...
func ParseFormat(s string) (Format, error) {
	...
	default:
		return FormatUnknown, fmt.Errorf("omnidoc: format %q: %w", s, ErrUnknownFormat)
	}
}
