...
func TestHTMLPagedMediaGolden(t *testing.T) {
	...
	for _, f := range pagedMediaGoldens {
		t.Run(f.name, func(t *testing.T) {
			doc, err := OpenHTMLBytes([]byte(f.html), WithDefaultPaged(), WithBundledFonts())
			if err != nil {
			...
			for i := 0; i < doc.PageCount(); i++ {
				...
				want := readPNG(t, path)
				if want == nil {
					t.Fatalf("missing golden %s; run: go test ./pkg/omnidoc -run TestHTMLPagedMediaGolden -update", path)
				}
