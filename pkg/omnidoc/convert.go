...
func Convert(ctx context.Context, in io.Reader, out io.Writer, opts ConvertOptions) error {
	...
	if err != nil {
		return fmt.Errorf("omnidoc: read input: %w", err)
	}
	from := opts.From
	if from == FormatUnknown {
		from = DetectFormat(data, "")
		if from == FormatUnknown {
			return fmt.Errorf("omnidoc: convert: cannot detect the input format (set ConvertOptions.From): %w", ErrUnknownFormat)
		}
	}
...
func ConvertFile(ctx context.Context, inPath, outPath string, opts ConvertOptions) (err error) {
	if opts.To == FormatUnknown {
		...
		if opts.To == FormatUnknown {
			return fmt.Errorf("omnidoc: convert to %q: cannot infer the output format from the extension (set ConvertOptions.To): %w", outPath, ErrUnknownFormat)
		}
	}
	...
	if isHTTPURL(inPath) {
		...
		if from != FormatUnknown && from != FormatHTML {
			return fmt.Errorf("omnidoc: convert %q: an http(s) URL is fetched as a web page (HTML), not %s: %w", inPath, from, ErrUnsupportedFormat)
		}
		from = FormatHTML
	} else {
		data, err = os.ReadFile(inPath)
		if err != nil {
			return fmt.Errorf("omnidoc: open %q: %w", inPath, err)
		}
		dir = filepath.Dir(inPath)
		if from == FormatUnknown {
			from = DetectFormat(data, inPath)
			if from == FormatUnknown {
				return fmt.Errorf("omnidoc: convert %q: cannot detect the input format (set ConvertOptions.From): %w", inPath, ErrUnknownFormat)
			}
		}
	}
	if err := CanConvert(from, opts.To); err != nil {
	...
	if err != nil {
		return fmt.Errorf("omnidoc: create %q: %w", outPath, err)
	}
	defer func() {
...
func (d *Document) Write(ctx context.Context, out io.Writer, to Format, opts ConvertOptions) error {
	...
	default:
		...
		return fmt.Errorf("omnidoc: write: no writer wired for %s output: %w", to, ErrUnsupportedFormat)
	}
}
