...
func todocxCmd(args []string) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), "usage: omnidoc todocx <input.pdf|.html|.md|.txt|URL> --out file.docx [flags]\n") //nolint:errcheck // stderr write
		fs.PrintDefaults()
	}
	...
	doc, err := openInput(input, omnidoc.FormatUnknown, "", *bundledFonts, false, nil, nil)
	if err != nil {
	...
	opts := omnidoc.ConvertOptions{DOCX: omnidoc.DOCXOptions{
		PageWidthPt:  *pageW,
		PageHeightPt: *pageH,
		MarginPt:     marginPt,
	}}
	...
	return doc.Write(context.Background(), f, omnidoc.FormatDOCX, opts)
}
