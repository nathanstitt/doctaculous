...
func rasterizeCmd(args []string) error {
	...
	imgFormat := omnidoc.FormatPNG
	switch *format {
	case "jpg", "jpeg":
		imgFormat = omnidoc.FormatJPEG
	case "webp":
		imgFormat = omnidoc.FormatWebP
	}
...
func htmlOpts(pageSize string, bundledFonts, printMedia bool) []omnidoc.HTMLOption {
	var opts []omnidoc.HTMLOption
	if pageSize == "letter" {
		opts = append(opts, omnidoc.WithPageSize(omnidoc.LetterWidthPt, omnidoc.LetterHeightPt))
	}
	if bundledFonts {
		opts = append(opts, omnidoc.WithBundledFonts())
	}
	if printMedia {
