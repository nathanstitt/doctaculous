...
func topdfCmd(args []string) error {
	...
	fs.Usage = func() {
		fmt.Fprintf(fs.Output(), "usage: omnidoc topdf <input.html|.docx|URL> --out file.pdf [flags]\n") //nolint:errcheck // stderr write
		fs.PrintDefaults()
	}
	...
	if err := doc.Write(context.Background(), f, omnidoc.FormatPDF, omnidoc.ConvertOptions{PDF: opts}); err != nil {
		return err
	}
	return nil
}
