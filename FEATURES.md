...
- `FormatFromMIME`/`Format.MIME()` strip params and case-fold. Explicit-Unknown pins cover legacy
  ...
  with `text/rtf` excepted. Those rows flip to PPTX/EPUB/RTF when those frontends land. The `OpenReader`/`OpenReaderAs(ctx,..)` stream entry points are fully
...
- **Animated WebP returns a typed `ErrAnimated`.** `x/image/webp` handles stills only, and its
  ...
  the canvas size with NO error, while `Decode` then fails with a bare `webp: invalid format` — the
  same error corrupt bytes produce. `pkg/webp` reads the flag upstream skips and returns
